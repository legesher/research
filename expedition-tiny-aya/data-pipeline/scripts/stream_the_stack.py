...
def is_test_file (file_path:str) -> bool:
    ...
    if filename.startswith("test_" or "Test_"):
        return True
...
def build_candidate(record: dict[str, Any], min_lines: int, max_lines: int, min_stars: int) -> CandidateRecord | None:
    ...
    if is_autogenerated(content) or is_mostly_comments(content) or has_no_definitions(content) or is_test_file(file_path):
...
def main() -> int:
    ...
    failed_quality = to_int(progress.get("failed_quality"))
