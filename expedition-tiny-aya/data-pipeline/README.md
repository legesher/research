...
The near-dedup LSH index is rebuilt in RAM on startup. At roughly 1M accepted records with `num_perm=128`, the stored hashvalues alone are on the order of 1 GB before the band index overhead, so larger runs should plan for that memory footprint.
