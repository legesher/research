...
def gather() -> dict:
    """Return {(condition, seed, template) -> {"orig": {cell: acc}, "rep": {cell: acc}, "orig_pf":..., "rep_pf":...}}."""
    out: dict[tuple, dict] = {}
...
for (cond, seed, template), cells in data.items():
    if cond == "baseline":
        for k, v in cells.items():
            # k = (template, bench, data, instr); template from k matches `template` from key
            baseline_cells[k] = v
...
def write_rollup(path, group_key_fn, key_label):
    g = defaultdict(list)
    for r in records:
...
write_rollup(
    OUT / "vs_baseline_by_condition.tsv", lambda r: r["condition"], "condition"
)
write_rollup(
